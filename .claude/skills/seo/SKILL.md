...
`${CLAUDE_PROJECT_DIR}/.claude/skills/seo` to the installed plugin directory, so the launcher is
...
`"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute
`"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" doctor --json`; its output
intentionally omits absolute paths and environment values. If any
`"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run` command reports that setup is
...
After any analysis command completes, offer to generate a PDF report via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run google_report.py`.
