...
- Raw HTML + rendered HTML (via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run render_page.py`)
...
- Core Web Vitals field data from CrUX (via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run pagespeed_check.py`)
...
- A PDF via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run google_report.py` when the user asks for one.
