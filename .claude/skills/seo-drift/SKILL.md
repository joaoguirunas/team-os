---
name: seo-drift
description: "Monitoramento de drift de SEO — captura baseline dos elementos críticos, detecta mudanças e rastreia regressões ao longo do tempo ('git para SEO'). Use ao pedir 'SEO drift', 'baseline', 'track changes', 'quebrou alguma coisa', 'SEO regression', 'antes e depois' ou 'deployment check'."
version: "2.3.1"
updated: "2026-09-25"
user-invocable: true
...
2. Fetch page via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run fetch_page.py <URL>`
3. Parse HTML via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run parse_html.py <URL>`
4. Optionally fetch CWV via `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run pagespeed_check.py <URL>` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_baseline.py <url>
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_baseline.py <url> --skip-cwv
```
...
**Execution:**
```bash
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_compare.py <url>
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_compare.py <url> --baseline-id 5
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_compare.py <url> --skip-cwv
```
...
```bash
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_report.py <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_history.py <url>
"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run drift_history.py <url> --limit 10
```
...
- **All URL fetching** goes through `"${CLAUDE_PROJECT_DIR}/.claude/skills/seo/scripts/claude-seo" run fetch_page.py`, which enforces SSRF protection
