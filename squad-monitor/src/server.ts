...
// Rotas: GET /, GET /events, POST /ingest, GET /agent/:id
// Porta 3099 por padrao (configuravel via PORT env)
...
import { processEvent, getSnapshot } from "./store.js";
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (method === "POST" && pathname === "/ingest") {
    // Responde imediatamente para nao bloquear Claude Code (RNF-04)
    res.writeHead(202, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ ok: true }));

    // Processa o evento de forma assincrona
    try {
      ...
      if (event) {
        ...
        console.log(`[ingest] ${event.kind} session=${event.sessionId.slice(0, 8)}${event.agentId ? " agent=" + event.agentId.slice(0, 8) : ""}`);
      }
    } catch (err) {
  ...
  // GET /agent/:id → detalhe do agente via transcript
  ...
  // GET /snapshot → snapshot atual (para debug)
...
server.listen(PORT, () => {
  console.log(`\nSquad Monitor rodando em http://localhost:${PORT}`);
  console.log(`   POST /ingest — recebe hooks do Claude Code`);
  console.log(`   GET  /events — SSE stream para o dashboard`);
  console.log(`   GET  /agent/:id?sessionId=...&cwd=... — detalhe de agente`);
  console.log(`\n   Configure hooks em ~/.claude/settings.json usando:`);
  console.log(`   hooks/settings.snippet.json\n`);
});
